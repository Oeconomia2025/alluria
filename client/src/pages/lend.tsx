...
// --- Mock Chart Data (kept — needs event indexer for real historical data) ---
...
function PieTooltip({ active, payload }: any) {
  ...
  const total = (payload[0] as any)._total || 1;
...
function DashboardContent() {
  ...
  // Protocol stats from live data
  const tvlDisplay = contractsDeployed && systemStats
    ? formatUSDDisplay(systemStats.totalCollateralValueUSD)
    : "—";
  const totalBorrowedDisplay = contractsDeployed && systemStats
    ? formatUSDDisplay(systemStats.totalALUDSupply)
    : "—";

  // Collateral ratio: TVL / total borrowed
  const tvlNum = systemStats ? Number(formatUSD(systemStats.totalCollateralValueUSD)) : 0;
  const borrowedNum = systemStats ? Number(formatUSD(systemStats.totalALUDSupply)) : 0;
  const collateralRatioPct = borrowedNum > 0 ? ((tvlNum / borrowedNum) * 100).toFixed(0) : "—";

  // Collateral distribution from live data
  const collateralDistribution = COLLATERAL_TOKENS.map((token, i) => {
    const stats = collateralStatsMap[token.address];
    const value = stats ? Number(formatUSD(stats.totalCollateralValueUSD)) : 0;
    const colors = ["#c43419", "#d4a853", "#8b4c42"];
    return { name: token.symbol, value, color: colors[i % colors.length] };
  }).filter((d) => d.value > 0);

  // If no live data, use placeholder for chart
  const hasLiveCollateral = collateralDistribution.length > 0;
  const pieData = hasLiveCollateral
  ...
  const userPositions = userTroves.map((trove) => {
          ...
          <div className="grid lg:grid-cols-[1fr_240px] gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <Card className="crypto-card border">
                  <CardContent className="p-4">
                    ...
                    <div className="text-xl font-bold text-white">{tvlDisplay}</div>
                ...
                <Card className="crypto-card border">
                  <CardContent className="p-4">
                    ...
                    <div className="text-xl font-bold text-white">{totalBorrowedDisplay}</div>
                    <div className="flex items-center gap-1 mt-1">
                ...
                <Card className="crypto-card border">
                  <CardContent className="p-4">
                    ...
                    <div className="text-xl font-bold text-green-400">{collateralRatioPct}%</div>
                    <div className="flex items-center gap-1 mt-1">
                ...
                <Card className="crypto-card border">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-muted-foreground">Stability Pool</span>
                    </div>
                ...
                <Card className="crypto-card border">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-muted-foreground">Min Collateral Ratio</span>
                    </div>
                    <div className="text-xl font-bold text-white">110%</div>
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="w-3 h-3 text-yellow-400" />
                      <span className="text-xs text-muted-foreground">liquidation threshold</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
                  ...
                  <>
                    ...
                    <Card className="crypto-card border">
                      <CardHeader className="pb-2">
                        ...
                        <div className="text-2xl font-bold text-white">{tvlDisplay}</div>
                      </CardHeader>
                      <CardContent className="px-2 pb-0">
                        <ChartContainer config={tvlChartConfig} className="h-[240px] w-full">
                          <AreaChart data={tvlData} margin={{ top: 5, right: 5, bottom: 0, left: -15 }}>
                            ...
                            <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} interval="preserveStartEnd" />
                            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} tickFormatter={(v) => `$${(v / 1_000_000).toFixed(0)}M`} width={45} />
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => [`$${(Number(value) / 1_000_000).toFixed(2)}M`, "TVL"]} className="border" style={{ backgroundColor: "#1a0d2e", borderColor: "#3d2050" } as any} />} />
                            <Area type="monotone" dataKey="tvl" stroke="#d4a853" strokeWidth={2} fill="url(#tvlGradient)" />
                          </AreaChart>
                        </ChartContainer>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent>
                        <ChartContainer config={pieChartConfig} className="h-[200px] w-full">
                          <PieChart>
                            ...
                            <Pie data={pieData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value" nameKey="name" strokeWidth={0}>
                              {pieData.map((entry) => (
                          ...
                          {pieData.map((item) => (
                            <div key={item.name} className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-white font-medium">
                                {item.value >= 1_000_000 ? `$${(item.value / 1_000_000).toFixed(1)}M` : `$${(item.value / 1_000).toFixed(1)}K`}
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      <CardContent className="px-2 pb-0">
                        <ChartContainer config={alurChartConfig} className="h-[240px] w-full">
                          <AreaChart data={alurData} margin={{ top: 5, right: 5, bottom: 0, left: -15 }}>
                            ...
                            <CartesianGrid strokeDasharray="3 3" stroke="#3d2050" vertical={false} />
                            <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} interval="preserveStartEnd" />
                            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} tickFormatter={(v) => `$${v.toFixed(3)}`} width={50} domain={["dataMin - 0.005", "dataMax + 0.005"]} />
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => [`$${Number(value).toFixed(4)}`, "ALUR"]} className="border" style={{ backgroundColor: "#1a0d2e", borderColor: "#3d2050" } as any} />} />
                            <Area type="monotone" dataKey="price" stroke="#d4a853" strokeWidth={2} fill="url(#alurGradient)" />
                          </AreaChart>
                        </ChartContainer>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="px-2 pb-0">
                        <ChartContainer config={aludChartConfig} className="h-[240px] w-full">
                          <AreaChart data={aludData} margin={{ top: 5, right: 5, bottom: 0, left: -15 }}>
                            ...
                            <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} interval="preserveStartEnd" />
                            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: "#6b7280" }} tickFormatter={(v) => `$${(v / 1_000_000).toFixed(0)}M`} width={45} />
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => [`$${(Number(value) / 1_000_000).toFixed(2)}M`, "Supply"]} className="border" style={{ backgroundColor: "#1a0d2e", borderColor: "#3d2050" } as any} />} />
                            <Area type="monotone" dataKey="supply" stroke="#c43419" strokeWidth={2} fill="url(#aludGradient)" />
                          </AreaChart>
                        </ChartContainer>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="px-2 pb-0">
                        <ChartContainer config={liquidationChartConfig} className="h-[240px] w-full">
                          <BarChart data={liquidationData} margin={{ top: 5, right: 5, bottom: 0, left: -15 }}>
                            ...
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => [value, "Liquidations"]} className="border" style={{ backgroundColor: "#1a0d2e", borderColor: "#3d2050" } as any} />} />
                      ...
                      <CardContent>
                        <ChartContainer config={liqPieChartConfig} className="h-[200px] w-full">
                          <PieChart>
                            ...
                            <Pie data={liquidationsByCollateral} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value" nameKey="name" strokeWidth={0}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card className="crypto-card border">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      {isConnected && (
                        <Badge variant="outline" className="text-xs">{userPositions.length} active</Badge>
                      )}
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {!isConnected ? (
                      ...
                      userPositions.map((pos, i) => (
                        <div key={i} className="p-3 border rounded-lg space-y-2 hover:bg-muted/50 transition-colors" style={{ borderColor: "#3d2050" }}>
                          <div className="flex items-center justify-between">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div>
                              ...
                              <div className="text-white font-medium">{pos.collateralAmount.toFixed(6)} {pos.collateral.symbol}</div>
                              <div className="text-muted-foreground">${pos.collateralValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
                            </div>
                            <div>
                              <span className="text-muted-foreground">Borrowed</span>
                              <div className="text-white font-medium">{pos.borrowedAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })} ALUD</div>
                          ...
                          <div className="flex items-center justify-between text-xs pt-1 border-t" style={{ borderColor: "#3d2050" }}>
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-orange-400 font-medium">${pos.liquidationPrice.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                            </div>
                          </div>
                        </div>
                      ))
                ...
                {/* Recent Protocol Activity (mock — needs event indexer) */}
                <Card className="crypto-card border">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs font-medium capitalize ${getActivityColor(event.type)}`}>{event.type}</span>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-white">{tvlDisplay}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">ALUD Circulating</span>
                      <span className="font-medium text-white">{totalBorrowedDisplay}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Utilization Rate</span>
                      <span className="font-medium" style={{ color: "#d4a853" }}>
                        {tvlNum > 0 ? ((borrowedNum / tvlNum) * 100).toFixed(1) : "—"}%
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
